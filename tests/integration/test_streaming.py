...
class TestRunnerIter:
    ...
    async def test_tool_error_events(self):
        """iter() emits ToolCallEvent with is_error for failures."""
        ...
        assert "boom" in tool_events[0].output
...
class TestSwarmIter:
    ...
    async def test_iter_emits_handoff_event(self):
        provider = MockProvider()
        a = Agent(
            name="a", description="A", system_prompt="A.", model="m",
            provider=provider, handoffs=[handoff("b", "Transfer to B")],
        )
        b = Agent(
            name="b", description="B", system_prompt="B.", model="m",
            provider=provider,
        )
        provider.responses = [
            make_tool_call_response(
                "transfer_to_b", {"message": "go to b"},
            ),
        ...
        # Agent A: handoff classified before execution (no ToolCallEvent)
        # Swarm emits HandoffEvent
...
class TestSSE:
    ...
    async def test_sse_format(self):
        ...
        assert frames[1].startswith("event: MessageEvent\n")
